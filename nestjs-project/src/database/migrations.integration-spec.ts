...
import { CreateVideos1782517179569 } from './migrations/1782517179569-CreateVideos';
...
describe('Database migrations (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    // Drop sequentially — parallel DROP ... CASCADE on FK-related tables
    // deadlocks against each other.
    for (const table of MANAGED_TABLES) {
      await dataSource.query(`DROP TABLE IF EXISTS "${table}" CASCADE`);
    }
    await dataSource.query(`DROP TABLE IF EXISTS "migrations" CASCADE`);
    // Enums survive a DROP TABLE; clear them so a fresh runMigrations succeeds
    // even when the shared DB was previously migrated.
  ...
  afterAll(async () => {
    // The revert tests leave token/video tables missing. Re-apply so the shared
    // DB is fully migrated when subsequent suites run.
